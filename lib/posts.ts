...
import {
  type Post,
  PostResponseSchema,
  PublicationPostListSchema,
  UserPostListSchema,
} from "@/lib/post-types";

export async function userPostsForId(
  id: number,
  opts?: {
    offset?: number;
    limit?: number;
  ...
  const response = await fetch(
    `https://substack.com/api/v1/profile/posts?profile_user_id=${id}${offset}${limit}`
  );
...
export async function publicationPostsForURL(
  url: string,
  opts?: {
    sort?: "new" | "top";
    offset?: number;
  ...
  const response = await fetch(
    `${urlParsed.data}/api/v1/archive?sort=${sort}${offset}${limit}`
  );
  const data = await response.json();
